...
import org.panacea.drmp.age.procedures.util.AGException;
import org.panacea.drmp.age.procedures.util.network_only.MyUniqueness;
...
@Slf4j
public class AllPathsToTargetNetworkOnly {
	...
	private static final MyUniqueness UNIQUENESS = MyUniqueness.NODE_HOSTNAME_PATH;//NODE_GLOBAL
	...
	// APOC 3.5.0.4
	public static Traverser traverse(TraversalDescription traversalDescription, Iterable<Node> startNodes, String pathFilter, String labelFilter, long minLevel, long maxLevel, MyUniqueness uniqueness, boolean bfs, boolean filterStartNode, EnumMap<NodeFilter, List<Node>> nodeFilter, String sequence, boolean beginSequenceAtStart) {
	...
	@Procedure(name = "agprocedures.allpathstotarget", mode = Mode.READ)
	@Description("Return all paths ending on the matched target nodes")
	public Stream<PathResult> allPathsTarget(@Name("start") Object start, @Name("config") Map<String, Object> config) throws Exception {
		...
		//FIXME: Remove filters for multilayer
		expandConfig.put("relationshipFilter", "<EXPLOIT");
		expandConfig.put("labelFilter", "+NDPrivilege");

		if(config.containsKey("initialCondition")){
			String condition = (String) config.get("initialCondition");
			//setting endnode label
			switch (condition) {
				case "user":
					expandConfig.put("labelFilter", ">NDUser");
					break;
				case "root":
					expandConfig.put("labelFilter", ">NDRoot");
					break;
				case "none":
					expandConfig.put("labelFilter", ">NDNone");
					break;
				default:
					throw new AGException("Must specify a valid initial condition betwen \"user\", \"root\", \"none\"");
			}


		}
		//Source specification via id
		if(config.containsKey("sourceId")){
			expandConfig.put("terminatorNodes",config.get("sourceId"));
			log.info("AllPathsToTargetNetworkOnly (ids) source: "+config.get("sourceId").toString() + " target: "+start.toString());

		}
		//Source specification via device hostnames (we must also follow edges to nodes with same device but increasing privileges)
		else if(config.containsKey("source")){
			expandConfig.put("endNodes",config.get("source"));
			log.info("AllPathsToTargetNetworkOnly source (hostnames): "+config.get("source").toString() + " target: "+start.toString());
		}
	...
	// APOC 3.5.0.4
	private Stream<Path> explorePathPrivate(Iterable<Node> startNodes, String pathFilter, String labelFilter, long minLevel, long maxLevel, boolean bfs, MyUniqueness uniqueness, boolean filterStartNode, long limit, EnumMap<NodeFilter, List<Node>> nodeFilter, String sequence, boolean beginSequenceAtStart) {
	...
	// APOC 3.5.0.4
	private MyUniqueness getUniqueness(String uniqueness) {
		MyUniqueness[] var2 = MyUniqueness.values();
		int var3 = var2.length;

		for(int var4 = 0; var4 < var3; ++var4) {
			MyUniqueness u = var2[var4];
